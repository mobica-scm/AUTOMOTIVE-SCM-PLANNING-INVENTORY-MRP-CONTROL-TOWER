...
| Open PO / shipment → scheduled receipt netting | Engine and schema support it (`POLine` → `ShipmentLine` → `Shipment.confidence`), but the source shipment log has no material-level line items to migrate from, so no seeded open POs exist yet — add one via the API/UI to see it net against demand |
